...
//    (only for CBC) Khởi tạo vector CBC
//    private byte[] iv;
    // Ma trận cần thiết cho AES(sBox + inverted one & rCon) 
    ...
    private void init(byte[] key) {
        ...
        // AES standard (4*32) = 128 bits Tiêu chuẩn AES
        Nb = 4;
        switch (key.length) {
            // 128 bit key
            case 16:
            ...
            case 32:
                ...
                break;
            default:
                throw new IllegalArgumentException("Chỉ hỗ trợ cho 128, 192 và 256 bit keys!");
        }

        // Tạo mảng lưu trữ cho các trạng thái.
        // Chỉ yêu cầu 2 trạng thái với 4 hàng và Nb cột.
        state = new int[2][4][Nb];

        // Vectơ lưu trữ để mở rộng tạo khóa.
        w = new int[Nb * (Nr + 1)];

        // Key expansion: Hàm mở rộng khóa
        expandKey();
    }

    // 128 bit của một trạng thái là phần bù XOR được áp dụng cho chúng với 128 bit của khóa được sử dụng.
    // s: ma trận trạng thái có Nb cột và 4 hàng.
    // Vòng: Một vòng của phím w sẽ được thêm vào.
    // s: trả về việc bổ sung khóa mỗi vòng
    ...
    // Cipher/Decipher methods : phương thức tạo mật mã và mã hóa
